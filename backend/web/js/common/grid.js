define(function (require, exports, module) {
    var Backbone = require('/js/lib/backbone/backbone');
    var _ = require('/js/lib/backbone/underscore');
    var Pagin = require('/js/lib/pagin');
    window.Template = require('/js/lib/template');
    ...
    jQuery.fn.grid = function (options) {
        var el = $(this),
            ...
            forceSearch = options.forceSearch || false, //当前页无数据时，是否加载前一页数据
            ...
            _pagin = null,
            loadPage = options.loadPage || null,
        ...
        var AppView = Backbone.View.extend({
            ...
            initialize: function () {
                ...
                this.getQueryParams();
                !noAutoload && grid.query(defaultPage, true);
            },
            initEvent: function () {
                var _this = this;
            ...
            // 加载后默认全选
            checkAllAfterLoad: function () {
                isCheckAll = true;
                this.$el.find('table thead .form-checkbox').addClass('selected');
                this.$el.find('table tbody .form-checkbox').addClass('selected');
                for (var i = 0; i < grid.models.length; i++) {
                    grid.models[i].trigger('toggleChecked', true);
                }
            },
            ...
            scrollPage: function (page) {
                ...
                if(this.page === undefined && page === undefined) return;
            ...
            render: function (collection, options) {
                ...
                var _pages = Math.ceil(options.total / options.pagesize),
                    _currpage = _curr.page;

                if (_pages < 1) {
                    _pages = 1;
                }

                if(forceSearch && options.total>0 && _currpage>1 && _currpage>_pages) {
                    grid.query(_pages, true);
                    return false;
                }
                ...
                loaded && loaded.call(this, options.response);
            },
