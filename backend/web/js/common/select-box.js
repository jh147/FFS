...
define(function (require, exports, module) {
     ...
     * 5、select1.disable()、select1.enable() 禁用、启用select实例
     * 6、select1.destroy() 销毁实例
     * 7、select1.refresh() 恢复select实例初始状态
     * */
    ...
    var SelectProto = {
            ...
            if (wrap) {
                ...
                wrap.on('click', 'dd', function () {
                    ...
                    if(!selectedEl.hasClass('on')){
                        var text = selectedEl.text(),
                            index = selectedEl.data('select-index'),
        ...
        createWrap: function ($node, isOrigin, config) {
            ...
            if (isOrigin) {
                ...
                if (!wrap.length) {
                    ...
                    //如果原生下拉框是禁用状态，复制
                    config.disabled = $node.prop('disabled');
                }
            } else {
                //div样式追加
        ...
        getMethods: function (instance) {
            ...
            methods.destroy = function () {
                _self.unbindEvent(instance);
                var boxId = SelectProto.getBoxId(instance.options.node);
                if (instance.originSelectSource) {
    ...
    var SelectBox = function (node, config) {
        this.config = $.extend(true, {node: $(node)}, defaultSetting, config);
        return SelectProto.init(this);
    };

    var create = function (node, config) {
    ...
    $.fn.extend({
        selectBox: function (config) {
            //暂时仅支持单选择器创建
            return create(this.eq(0), config);
        },
        selectBoxs: function (config) {
            this.each(function () {
