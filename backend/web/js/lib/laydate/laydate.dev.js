...
//全局配置，如果采用默认均不需要改动
var config =  {
    path: '/js/lib/laydate/', //laydate所在路径
...
Dates.timeVoid = function(times, index){
    if(Dates.ymd[1]+1 == Dates.mins[1] && Dates.ymd[2] == Dates.mins[2]){
        if(index === 0 && (times < Dates.mins[3])){
            return 1;
        } else if(index === 1 && Dates.hmsin[0].value === Dates.mins[3] && times < Dates.mins[4]){
            return 1;
        } else if(index === 2 && Dates.hmsin[1].value === Dates.mins[4] && times < Dates.mins[5]){
            return 1;
        }
...
Dates.orien = function(obj, pos){
    ...
    if(rect.bottom + obj.offsetHeight <= Dates.winarea()){
        tops = rect.bottom - 1;
    } else {
        tops = rect.top > obj.offsetHeight ? rect.top - obj.offsetHeight + 1 : Dates.winarea() - obj.offsetHeight;
    }
...
Dates.events = function(){
    ...
    log.tabMonth = function(type){
        ...
        /* 切换月份问题修改 */
        var date_len = new Date(Dates.ymd[0] , (Dates.ymd[1] + 1) , 0).getDate(),
            date_save = Dates.saveDate,
            date_set = Math.max(date_save , Dates.ymd[2]);
        date_set = Math.min(date_set , date_len);
        /* end */

        Dates.viewDate(Dates.ymd[0], Dates.ymd[1], date_set);
    };
    ...
    Dates.each(S('#laydate_ms span'), function(i, elem){
        Dates.on(elem, 'click', function(ev){
            ...
            if(!Dates.hasClass(this, as[1])){
                /* 切换月份问题修改 */
                var date_len = new Date(Dates.ymd[0] , ((this.getAttribute('m')|0) + 1) , 0).getDate(),
                    date_save = Dates.saveDate,
                    date_set = Math.max(date_save , Dates.ymd[2]);
                date_set = Math.min(date_set , date_len);
                /* end */
                Dates.viewDate(Dates.ymd[0], this.getAttribute('m')|0, date_set);
            }
        });
    });
