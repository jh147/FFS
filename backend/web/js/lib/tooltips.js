...
define(function(require, exports, module) {
    ...
    $.pt = $.pureToolTips = function(options) {
        ...
        var opts = $.extend({
            ...
            forcePos    : '',       //强制设置position的值，因为后面的代码可能会计算 -> 重置传入的position
        ...
        body = $('body'),
        position = opts.forcePos || opts.position,
        align = opts.align,
        arrow = opts.arrow,
        ...
        function init() {
            ...
            $pt.removeClass().addClass(opts.skin + ' pt ' + (arrow ? arrowClass : '')).find('.cont').html(opts.content).css({width:opts.width, height:opts.height});
            ...
            if(position === 'b' && !opts.forcePos && (body.height() - top + $(document).scrollTop()) - h <= 0 && top >= h){
                position = 't';
                arrowClass = constant.t;
                return init();
            }
