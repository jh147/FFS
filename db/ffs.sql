...
CREATE TABLE IF NOT EXISTS `shipping_order` (
  ...
  `actual_weight` int(4) NOT NULL DEFAULT '0' COMMENT '实际重量',
  `billing_weight` int(4) NOT NULL DEFAULT '0' COMMENT '计费重量',
  `freight_rates` decimal(14,2) NOT NULL DEFAULT '0.00' COMMENT '费率（净运价）',
  `freight_fee` decimal(14,2) NOT NULL DEFAULT '0.00' COMMENT '航空运费',
  `fuel_fee` decimal(14,2) NOT NULL DEFAULT '0.00' COMMENT '燃油费',
  `freight_total_fee` decimal(14,2) NOT NULL DEFAULT '0.00' COMMENT '运费总额（含燃油）',
  `pg_quantity` int(4) NOT NULL DEFAULT '0' COMMENT '拉货件数',
  `pg_weight` int(4) NOT NULL DEFAULT '0' COMMENT '拉货重量',
  `pg_freight_rates` decimal(14,2) NOT NULL DEFAULT '0.00' COMMENT '拉货费率',
  `pg_loss_fee` decimal(14,2) NOT NULL DEFAULT '0.00' COMMENT '拉货损失金额',
  `pg_reason` varchar(255) NOT NULL COMMENT '拉货原因',
  `pg_processing_method` varchar(100) NOT NULL COMMENT '拉货处理方式',
  `pg_remark` varchar(255) NOT NULL COMMENT '拉货备注',
  `created_on` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '记录创建时间',
  `created_by` char(36) DEFAULT NULL COMMENT '记录创建者',
  `modified_on` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '记录修改时间',
  `modified_by` char(36) DEFAULT NULL COMMENT '记录修改者',
